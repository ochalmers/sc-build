...
function countExamples() {
  return INSPIRATION_CATEGORIES.reduce((n, cat) => n + (cat.examples?.length ?? 0), 0);
}
      ...
      <main className="pt-[6.5rem]">
            ...
            >
              ...
              <dl className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-2xl border border-ink-200/80 bg-white/70 p-5">
                  ...
                  <dd className="mt-1 text-[12px] text-ink-500">Across {categoryCount} design tasks</dd>
                </div>
                <div className="rounded-2xl border border-ink-200/80 bg-white/70 p-5">
                  <dt className="text-[10px] font-medium uppercase tracking-[0.14em] text-ink-500">
                    Reference examples
                  ...
                  <dd className="mt-1 text-[12px] text-ink-500">With what/relevance notes</dd>
                </div>
                <div className="rounded-2xl border border-ink-200/80 bg-white/70 p-5">
                  <dt className="text-[10px] font-medium uppercase tracking-[0.14em] text-ink-500">
                    PRD surfaces
                  </dt>
                  <dd className="mt-2 text-[28px] font-medium tracking-tight text-ink-950">14</dd>
                  <dd className="mt-1 text-[12px] text-ink-500">Public Visitor + Listener mapped</dd>
                </div>
              </dl>
        ...
        {INSPIRATION_CATEGORIES.map((category) => {
          ...
          return (
            ...
            >
              <p className="mb-4 max-w-2xl rounded-xl border border-ink-200/60 bg-paper-100/60 px-4 py-3 text-[12px] text-ink-600">
                <span className="font-medium text-ink-800">What we are doing · </span>
                {category.whatWeAreDoing}
              </p>
              <p className="mb-10 max-w-2xl rounded-xl border border-ink-200/60 bg-paper-100/60 px-4 py-3 text-[12px] text-ink-600">
                <span className="font-medium text-ink-800">PRD mapping · </span>
              ...
              <ReferenceList examples={category.examples} />
            </FlowSection>
          );
        })}
